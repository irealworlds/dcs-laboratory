...
public class TwoXTwoTable {
    ...
    public TwoXTwoTable(Map<FuzzyValue, Map<FuzzyValue, FuzzyValue>> firstTable,
                        Map<FuzzyValue, Map<FuzzyValue, FuzzyValue>> secondTable) {
    ...
    public FuzzyToken[] execute(FuzzyToken firstToken, FuzzyToken secondToken) {
        ...
        for (FuzzyValue firstTokenNonZero : firstToken.getNonZeroValues()) {
            for (FuzzyValue secondNonZero : secondToken.getNonZeroValues()) {
                FuzzyValue firstConclision = firstTable.get(firstTokenNonZero).get(secondNonZero);
                FuzzyValue secondConclusion = secondTable.get(firstTokenNonZero).get(secondNonZero);

                double valueOfConclision = firstToken.getFuzzyValue(firstTokenNonZero)
